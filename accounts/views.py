...
def export_excel_view(request):
    ...
    if company_phone:
        ws.append([f"Phone: {company_phone}"])
    ws.append([f"Statement - Generated on {timezone.now().strftime('%Y-%m-%d %H:%M')}"])
    ...
    # Write promotional signature at the end
    ws.append([])
    ws.append([])
    ws.append(["Statement Generated by App"])
    ws.append(["Developed and updated by:"])
    ws.append(["CGPLUX STUDIOS"])
    ws.append(["https://cgpluxstudios.com/"])
    ws.append(["Call us: +92 319 1086099"])
    ws.append(["Email us: info@cgpluxstudios.com"])
...
def export_pdf_view(request):
    ...
    company_address = company.address if company and company.address else ""
    company_phone = company.phone if company and company.phone else ""
    generated_text = f"Statement - Generated on {timezone.now().strftime('%Y-%m-%d %H:%M')}"
    ...
    x_text = 130 if logo_drawn else 40
    pdf.drawString(x_text, y + 10, company_name)
    pdf.setFont("Helvetica", 9)
    pdf.setFillColor(colors.HexColor('#64748b'))
    
    curr_y = y - 5
    if company_address:
        pdf.drawString(x_text, curr_y, company_address)
        curr_y -= 12
    if company_phone:
        pdf.drawString(x_text, curr_y, f"Phone: {company_phone}")
        curr_y -= 12
    pdf.drawString(x_text, curr_y, generated_text)
    ...
    # Check space at the bottom, if less than 110, add page
    if y < 110:
        pdf.showPage()
        y = height - 55
    ...
    pdf.rect(40, y - 50, width - 80, 65, fill=True, stroke=True)
    
    pdf.setFillColor(colors.HexColor('#64748b'))
    pdf.setFont("Helvetica-Oblique", 8)
    pdf.drawCentredString(width / 2, y + 2, "Statement Generated by App")
    pdf.setFont("Helvetica-Bold", 8)
    pdf.drawCentredString(width / 2, y - 8, "Developed and updated by:")
    pdf.setFillColor(colors.black)
    pdf.setFont("Helvetica-Bold", 10)
    pdf.drawCentredString(width / 2, y - 20, "CGPLUX STUDIOS")
    pdf.setFillColor(colors.HexColor('#3b82f6'))
    pdf.setFont("Helvetica", 8)
    pdf.drawCentredString(width / 2, y - 30, "https://cgpluxstudios.com/")
    pdf.setFillColor(colors.HexColor('#64748b'))
    pdf.setFont("Helvetica", 8)
    pdf.drawCentredString(width / 2, y - 42, "Call us: +92 319 1086099   |   Email us: info@cgpluxstudios.com")
...
def export_sales_pdf_view(request):
    ...
    if company_phone:
        story.append(Paragraph(f"Phone: {company_phone}", subtitle_style))
    story.append(Paragraph("Statement - Generated on " + timezone.now().strftime("%Y-%m-%d %H:%M"), subtitle_style))
    ...
    promotional_style = ParagraphStyle(
        'PromoText',
        parent=styles['Normal'],
    ...
    promo_text = """
    Statement Generated by App<br/>
    <b>Developed and updated by:</b><br/>
    <font color="#000000"><b>CGPLUX STUDIOS</b></font><br/>
    <u>https://cgpluxstudios.com/</u><br/><br/>
    <b>Call us:</b> +92 319 1086099 | <b>Email us:</b> info@cgpluxstudios.com
    """
    
    story.append(Spacer(1, 20))
    promo_table = Table([[Paragraph(promo_text, promotional_style)]], colWidths=[500])
